---
name: Issue Tracker
description: GitHub/ GitLab issue management — create, label, comment, close via CLI
type: passive
match: ""
---
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.knightframe/.env ] && grep -q "^GITHUB_TOKEN=" ~/.knightframe/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.knightframe/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
