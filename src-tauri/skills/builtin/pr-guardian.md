---
name: PR Guardian
description: Auto-track pull requests — poll reviews, CI status, and merge readiness
type: passive
match: ""
---
...
## Objective
Guard a pull request persistently until one of these terminal outcomes occurs:
...
15. Maintain terminal/session ownership: while guarding is active, keep consuming watcher output in the same turn; do not leave a detached `--watch` process running and then end the turn as if monitoring were complete.
...
```bash
python3 .knightframe/skills/pr-guardian/scripts/gh_pr_watch.py --pr auto --once
```

### Continuous watch (JSONL)

```bash
python3 .knightframe/skills/pr-guardian/scripts/gh_pr_watch.py --pr auto --watch
```

### Trigger flaky retry cycle (only when watcher indicates)

```bash
python3 .knightframe/skills/pr-guardian/scripts/gh_pr_watch.py --pr auto --retry-failed-now
```

### Explicit PR target

```bash
python3 .knightframe/skills/pr-guardian/scripts/gh_pr_watch.py --pr <number-or-url> --once
```
...
- `gh api repos/<owner>/<repo>/actions/jobs/<job-id>/logs > /tmp/knightframe-gh-job-<job-id>-logs.zip`
...
Read `.knightframe/skills/pr-guardian/references/heuristics.md` for a concise checklist.
...
It intentionally surfaces KnightFrame reviewer bot feedback in addition to human reviewer feedback. Most unrelated bot noise should still be ignored.
For safety, the watcher only auto-surfaces trusted human review authors (for example repo OWNER/MEMBER/COLLABORATOR, plus the authenticated operator) and approved review bots such as KnightFrame.
...
2. Commit with `knightframe: address PR review feedback (#<n>)`.
...
You can resolve review comment threads from the human who requested guarding or from the KnightFrame
review bot. When resolving, leave a comment prefixed with `[from KnightFrame]: ` and explain what changes
you made and which commit includes them. Don't touch review threads if other humans other than the
user who requested guarding have participated.
...
- `knightframe: fix CI failure on PR #<n>`
- `knightframe: address PR review feedback (#<n>)`

## Monitoring Loop Pattern
Use this loop in a live KnightFrame session:
...
When the user explicitly asks to monitor/watch/guard a PR, prefer `--watch` so polling continues autonomously in one command. Use repeated `--once` snapshots only for debugging, local testing, or when the user explicitly asks for a one-shot check.
Do not stop to ask the user whether to continue polling; continue autonomously until a strict stop condition is met or the user explicitly interrupts.
Do not hand control back to the user after a review-fix push just because a new SHA was created; restarting the watcher and re-entering the poll loop is part of the same guarding task.
If a `--watch` process is still running and no strict stop condition has been reached, the guarding task is still in progress; keep streaming/consuming watcher output instead of ending the turn.
...
- User intervention is required and KnightFrame cannot safely proceed alone.
...
- Treat push confirmations, intermediate CI snapshots, ready-to-merge snapshots, and review-action updates as progress updates only; do not emit the final summary or end the guarding session unless a strict stop condition is met.
...
- Heuristics and decision tree: `.knightframe/skills/pr-guardian/references/heuristics.md`
- GitHub CLI/API details used by the watcher: `.knightframe/skills/pr-guardian/references/github-api-notes.md`
